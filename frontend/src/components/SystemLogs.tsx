  ...
  return (
    <div className="flex flex-col h-full max-w-4xl mx-auto w-full pt-4">
      ...
      <div className="flex items-center gap-4 mb-4">
        <form onSubmit={runCommand} className="flex gap-2">
            <select 
              name="ip"
              required
              className="bg-app-card border border-app-border rounded flex items-center px-3 py-1.5 text-xs text-app-text focus:outline-none focus:border-app-accent font-mono min-w-[200px]"
              defaultValue=""
            >
      ...
      <div className="flex-1 bg-[#000] border-t border-x border-app-border rounded-t-lg p-5 font-mono text-[12px] leading-relaxed overflow-y-auto">
        ...
        {logs.length === 0 && (
            <div className="text-app-muted/50 italic">Execute a command to view output...</div>
        )}
        ...
        <div className="space-y-1">
          ...
          <div className="mt-2 text-app-text">
            ...
            {loading ? <span className="animate-pulse ml-1 opacity-50 block w-2 h-4 bg-app-accent inline-block"></span> : <span className="border-l-2 border-app-accent pl-1 ml-1 text-transparent">_</span>}
          </div>
        </div>
      </div>
    </div>
